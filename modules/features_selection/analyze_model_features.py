# modules/feature_selection/analyze_model_features.py (exemple de nom de fichier)
"""
Module pour l'analyse approfondie de l'importance des variables.
Intègre RFECV, Permutation Importance et SHAP.
"""
...
def analyze_feature_importance(model, X_train, y_train, X_eval, y_eval, feature_names,
    ...
    Permutation Importance et SHAP (si disponible).
    ...
    if 'shap' in methods_to_run:
        try:
            ...
            # Réduire la taille de l’échantillon pour limiter l’usage mémoire
            n_samples = min(100, X_eval.shape[0])
            X_sample = X_eval[:n_samples]
...
# results_gradboost = analyze_feature_importance(
#     model=pipeline_gradboost_mice.named_steps['clf'], # Le modèle de base
#     X_train=X_scaled_mice, # Données prétraitées sans SMOTE
#     y_train=y_train_mice,
#     X_eval=X_test_mice_processed, # Ou X_val_mice_processed
#     y_eval=y_test_mice,
#     feature_names=X_train_mice.columns.tolist(),
#     method='all', # Appliquer RFECV, Permutation, SHAP
#     cv_folds=5,
#     n_repeats_perm=20,
#     output_dir=OUTPUTS_DIR / "feature_selection" / "gradboost_mice",
#     model_name="GradBoost_MICE_Optimized"
# )